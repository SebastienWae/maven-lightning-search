...
import {
	type ApiResponse,
	ApiResponseSchema,
	type PublishedContentPageSection,
	type WorkshopItem,
} from "./schema";
...
export async function scrapeWorkshops(): Promise<WorkshopItem[]> {
	...
	console.log(
		`Page 1: Fetched ${firstPage.items.length} items (Total pages: ${firstPage.metadata.pages})`,
	);
	...
	for (let page = 2; page <= totalPages; page++) {
		...
		try {
			...
			console.log(
				`Page ${page}: Fetched ${pageData.items.length} items (Total so far: ${allItems.length})`,
			);
		} catch (error) {
...
async function getOrCreateInstructor(
	...
	const existing = db
		.select()
		.from(instructor)
		.where(eq(instructor.id, identityHash))
		.limit(1)
		.get();
	if (existing) {
		await db
 ...
 */
export async function saveWorkshopsToDatabase(
	...
	for (const item of workshops) {
		...
		const mainSection = page.sections.find(
			(section) => section.section_type === "main",
		);
		if (!mainSection) {
		...
		for (const { name, image_url } of mainSection.instructor_infos) {
			...
			const existingRelation = db
				...
				.where(
					and(
						eq(workshopInstructors.workshopId, item.id),
		...
		for (const tag of item.workshop_tags) {
			...
			const existingRelation = db
				...
				.where(
					and(
						eq(workshopTags.workshopId, item.id),
						eq(workshopTags.tagId, tagId),
					),
				)
				.limit(1)
				.get();
	...
	console.log(
		`Saved ${savedCount} new workshops, updated ${updatedCount} existing workshops`,
	);
}
